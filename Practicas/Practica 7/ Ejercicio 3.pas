{ Una remisería dispone de información acerca de los viajes realizados durante el mes de mayo de
2020. De cada viaje se conoce: número de viaje, código de auto, dirección de origen, dirección de
destino y kilómetros recorridos durante el viaje. Esta información **se encuentra ordenada** por código de auto y para un mismo código de auto pueden existir 1 o más viajes. Se pide:

1. Informar los dos códigos de auto que más kilómetros recorrieron.
2. Generar una lista nueva con los viajes de más de 5 kilómetros recorridos, ordenada por número
de viaje.
}
